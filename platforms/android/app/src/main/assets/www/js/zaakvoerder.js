...
function getListHistory() {    
  ...
  db.collection('Events').where('Status' , '==', "afgekeurd").limit(5).get().then((snapshot)=>{
    snapshot.docs.forEach(doc => {              
      var tlines = "";          
      tlines += "<li><a href='/aanvraagafkeurenuitlegReadZaakvoerder/' class='item-link item-content eigenafgekeurdeevents' id=" + doc.id + "><div class='item-media'><i class='f7-icons'>multiply_circle</i></div><div class='item-inner'><div class='item-title'>" + doc.data().Eventnaam + "</div></div></a></li>";
      $$("#voorstellenhistory").append(tlines);       
  })
  })      
}
  ...
  function getEvents(){
    ...
    var tlines = ""; 
      ...
      db.collection('Events').where('Status' , '==', "gepland").get().then((snapshot)=>{
        snapshot.docs.forEach(doc => {    
          tlines += "<li><a href='/finaleventzaakvoerder/' class='finaleventlinks' id=" + doc.id + ">" + doc.data().Eventnaam + "</a></li>";       
      })
...
// om de data bij een voorstel op te halen
function getshowFinalEventInfoData(){    
  db.collection("Events").doc(eventnummer)
  .get()
  .then(function(doc) {        
    $$("#fInfo").html('<b>Info:</b> ' + doc.data().Beschrijving); 
    $$("#fEventnaam").html(doc.data().Eventnaam); 
    $$("#fStartuur").html('<b>Tijdstip:</b> ' + doc.data().Startuur); 
    $$("#fDuur").html('<b>Duurtijd:</b> ' + doc.data().Duurtijd); 
    $$("#fLocatie").html('<b>Locatie:</b> ' + doc.data().Locatie); 
    $$("#fDatum").html('<b>Datum:</b> ' + timestampToDate(doc.data().Datum)); 
    $$("#fDeelnemers").html('<b>Aantal deelnemers:</b> ' + aantalDeelnemers + '/' + getAantalPersoneel()); 
    $$("#fURL").html('<b>URL:</b> ' + doc.data().URL); 
    document.getElementById("fImg").setAttribute("src", doc.data().Img);
    document.getElementById("fURL").setAttribute("href", doc.data().URL);
...
$$(document).on('click', 'a.finaleventlinks', function (e) {
  eventnummer = $$(this).attr("id");
  console.log('link clicked'); 
  getshowFinalEventInfoData();  
});
